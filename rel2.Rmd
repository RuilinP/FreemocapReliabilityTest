...
```{r}
ggplot(df, aes(time, com_left_foot_x)) +
  geom_line(color="red") + 
  ggtitle("Freemocap x coordinate")

ggplot(df, aes(time, com_left_foot_y)) +
  geom_line(color="green") + 
  ggtitle("Freemocap y coordinate")

ggplot(df, aes(time, com_left_foot_z)) +
  geom_line(color="blue") + 
...
```{r}
fwrite(df, "leftfoot_freemocap_5min.csv")
```
...
```{r}
df[, mocap_sig := sqrt(
  c(NA, diff(com_left_foot_x))^2 +
  c(NA, diff(com_left_foot_y))^2 +
  c(NA, diff(com_left_foot_z))^2
)]
